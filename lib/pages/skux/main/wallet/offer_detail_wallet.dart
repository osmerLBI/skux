...
class OfferDetailWalletPage extends StatefulWidget {
  const OfferDetailWalletPage({
    Key key,
    this.offer,
  ...
  @override
  State<OfferDetailWalletPage> createState() => _OfferDetailWalletPageState();
}

class _OfferDetailWalletPageState extends EventHubState<OfferDetailWalletPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    Util.postFrame((p0) async {
      await SkuxInfo.writeRecentViewed(widget.offer['offer']['uuid']);
    });
  }
          ...
          children: [
            SizedBox(
              height: MediaQuery.of(context).size.height -
                  ...
                  100,
              child: WebViewPage(
                url: getWebViewUrl(widget.offer['offer']['contentUrl']).toString(),
                onLoadFinished: () {
                  setState(() {
                    _showButtons = true;
                  });
            ...
            if ((_showButtons == true) && (widget.offer['offer']['status'] == 'REDEEMED'))
              Padding(
            ...
            if ((_showButtons == true) && (widget.offer['offer']['status'] == 'ACTIVE'))
              Padding(
                padding: const EdgeInsets.only(top: 10),
                child: Semantics(
                  sortKey: const OrdinalSortKey(1),
                  ...
                  child: ClaimOfferButtonMutation(
                    offerUuid: widget.offer['offer']['uuid'],
                    ...
                    isDisabled: widget.offer['offer']['status'] != 'ACTIVE',
                  ),
                ),
              ),
          ],
